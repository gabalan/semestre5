...
bool test_new_game_ext(){
  ...
  assert(jeu);
  if(game_width(jeu) != NB_COL || game_height != NB_LINE){
...
bool test_is_game_over (){
      ...
      if(is_game_over(jeu) == false){
        fprintf(stderr,"error in is_game_over \n");
        rep = false;
      }

      add_mirror_ext(jeu, ANTIMIRROR, 0, 3);
      add_mirror(jeu, MIRROR, 3, 3);
      add_mirror(jeu,ANTIMIRROR, 0, 2);
      add_mirror(jeu, ANTIMIRROR, 2, 1);
      add_mirror(jeu, MIRROR, 3, 1);
      add_mirror(jeu, ANTIMIRROR, 0, 0);
      add_mirror(jeu, ANTIMIRROR, 3, 0);
