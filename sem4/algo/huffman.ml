...
let rec insert_sorted c l=match l with
  ...
  (*2*)
   ...
   let inf2 x y =if x<y then -1 else if x=y then 0 else 1;;
    ...
    let codelist t=
      let rec aux t path acc=
        match t with
        |Single c->[c,List.rev path]
        |Double(l,r)->let lacc=aux l (0::path) acc in
                      let racc=aux r (1::path) acc in lacc @ acc @ racc in aux t [] [];;

      let t=string_to_hufftree "SWIMMING IN THE MISSISSIPPI";;


      (*7*)
      let rec find_code c l=
        match l with
        |[]->failwith"liste vide"
        |(x,code)::r->if x=c then code
                                    else find_code c r;;

      let encode  s=
        let t=codelist (string_to_hufftree s) in
        let rec aux  i n s acc=
          if i=n then acc
          else
            aux (i+1) n s (acc@(find_code s.[i] t)) in aux 0 (String.length s) s [];;


        (*Exercice decodage*)

      let list_to_string l=
        let buf=Buffer.create(List.length l) in List.iter(Buffer.add_char buf);
                                                Buffer.contents buf;;
